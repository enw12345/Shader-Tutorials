Shader "Tutorial/017_GenerateWave_Postprocessing"
{
	//show values to edit in inspector
	Properties{
		[HideInInspector] _MainTex("Texture", 2D) = "white" {}
	[Header(Wave)]
	_WaveDistance("Distance from the player", float) = 10
	...
	//depth as distance from the camera in units
	depth = depth * _ProjectionParams.z;

	//get source color
	fixed4 source = tex2D(_MainTex, i.uv);
	//skip waveand return source color if we are at the skybox
