...
public class BakeTextureWindow : EditorWindow
{
    ...
    string FileField(string path)
    {
        ...
        using(new GUILayout.HorizontalScope())
        {
            ...
            {
                //setdefault values for the directory, then try to override them with values of existing path
                string directory = "Assets";
                string fileName = "MateruakImage.png";
                try
                {
                ...
                string chosenFile = EditorUtility.SaveFilePanelInProject("Choose image file", fileName, "png", "Please enter a file name to save the image to");
                if (!string.IsNullOrEmpty(chosenFile))
                {
                    path = chosenFile;
                }
                //repaint editor because the file changed and we can't set it in the textfield
                Repaint();
            }
        }
